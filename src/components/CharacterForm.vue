...
<!--    </div>-->
    ...
    <div class="form-group" :class="{ 'form-group--error': $v.charactername.$error }">
      ...
      <input class="form__input" placeholder="enter some charactername here" v-model.trim="$v.charactername.$model"/>
    </div>
...
export default {
  ...
  data () {
    return {
      ...
      // paymenttype: this.character.paymenttype,
      ...
      if (this.$v.$invalid) {
        ...
        setTimeout(() => {
          ...
          console.log('Submitting in CharacterForm : ' +
              JSON.stringify(this.character, null, 5))
          this.$emit('character-is-created-updated', this.character)
        }, 500)
      }
    }
  }
